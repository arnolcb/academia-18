...
const cargarCurso = async () => {
  ...
  try {
    ...
    const cursoDoc = await getDoc(doc(db, 'cursos4', cursoId));
    if (!cursoDoc.exists()) {
    ...
    const semanasRef = collection(db, `cursos4/${cursoId}/semanas`);
    ...
    for (const semanaDoc of semanasSnapshot.docs) {
      ...
      const recursosRef = collection(db, `cursos4/${cursoId}/semanas/${semanaDoc.id}/recursos`);
